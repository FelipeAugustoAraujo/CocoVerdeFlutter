...
import 'package:Cocoverde/shared/exceptions/app_exception.dart';

import '../../environment.dart';

class HttpUtils {
  static String errorHeader = 'x-cocoverdeapp-error';
  ...
  static Future<Response> postRequest<T>(String endpoint, T body) async {
    ...
    Response? response;
    try {
      response = await http.post(Uri.parse('${Constants.api}$endpoint'), headers: headers, body: body.toString(),
  ...
  static Future<Response> putRequest<T>(String endpoint, String body, int id) async {
    var headers = await HttpUtils.headers();
    //final String json = JsonMapper.serialize(body, SerializationOptions(indent: ''));
    Response response;
    try {
      response = await http.put(Uri.parse('${Constants.api}$endpoint'), headers: headers, body: body,
          encoding: Encoding.getByName('utf-8')).timeout(Duration(seconds: timeout));
    } on SocketException {
      throw FetchDataException('No Internet connection');
    } on TimeoutException {
